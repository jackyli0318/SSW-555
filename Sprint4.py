# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 15:02:27 2018
...
def Siblings_Spacing(tmp_fam, fam_dict, indi_dict):
    children = fam_dict[tmp_fam]['children']
    for i in range(len(children)):
        dobi = datetime.datetime.strptime(indi_dict[children[i]]['dob'],"%Y-%m-%d")
        for j in range(i + 1, len(children)):
            dobj = datetime.datetime.strptime(indi_dict[children[j]]['dob'],"%Y-%m-%d")
            if 2 <= abs((dobi - dobj).days) <= 30.4 * 8:
                return False
    return True


def Multiple_Births(tmp_fam, fam_dict, indi_dict):
    children = fam_dict[tmp_fam]['children']
    d = {}
    for child in children:
        dob = datetime.datetime.strptime(indi_dict[child]['dob'],"%Y-%m-%d")
        if dob in d:
            d[dob] += 1
        else:
            d[dob] = 1
